...
public class SoloPVPExecutorTest {
    ...
    @Test
    public void oneMatchOverTest_30() throws NoSuchFieldException, IllegalAccessException {
        Player loser = mock(Player.class);
        when(loser.getName()).thenReturn("kary");
        Player winner = mock(Player.class);
        when(winner.getName()).thenReturn("kary1");
        ...
        assertEquals(30,playersScoreGainAndMatchStartTime.get (winner)[0]);
        assertEquals (-10,playersScoreGainAndMatchStartTime.get (loser)[0]);
    }
